...
REQUEST_RULES = """너는 리그 오브 레전드 서포터 픽 추천 분석기다.

아래 DRAFT_SNAPSHOT_V2 데이터와 OP.GG 데이터를 사용하여
현재 시점에서 선택하기 좋은 서포터 챔피언을 정확히 3개 추천하라.

[판단 규칙]
1. LOCKED는 확정된 챔피언이다.
2. HOVER는 픽 의사일 뿐 확정된 챔피언이 아니다.
3. UNKNOWN 정보는 임의로 추측하지 않는다.
4. 상대 서포터 source가 MANUAL_UNKNOWN이거나 UNKNOWN이면 블라인드 픽 안정성을 우선한다.
...
12. 서포터 포지션에서 실제로 사용할 가치가 있는 챔피언만 추천한다.
